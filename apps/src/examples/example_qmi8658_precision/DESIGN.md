...
在嵌入式系统中，I2C总线扫描和通信时如果设备不存在或响应异常，会导致：
- **看门狗超时复位** - 无限等待循环触发硬件看门狗
- **启动延迟严重** - 每个不存在的地址都要等待超时
- **用户体验差** - 开机时间长达数秒

### A.2 优化方案

#### 1. 硬件I2C超时保护（iic_hw.c）

在 `fw-AC63_BT_SDK/cpu/bd19/iic_hw.c` 中为所有无限等待循环添加超时机制：

```c
// 统一超时配置宏
#define IIC_TIMEOUT_STOP      5000   // STOP信号超时（约0.5ms）
#define IIC_TIMEOUT_TX        10000  // 发送字节超时（约1ms）
#define IIC_TIMEOUT_RX        10000  // 接收字节超时（约1ms）
#define IIC_TIMEOUT_BUF       10000  // 批量读写超时（约1ms）
```

**修改位置：**
- `hw_iic_stop()` - STOP信号等待
- `hw_iic_tx_byte()` - 单字节发送
- `hw_iic_rx_byte()` - 单字节接收
- `hw_iic_write_buf()` - 批量写入
- `hw_iic_read_buf()` - 批量读取
...
#### 2. I2C扫描范围优化（i2c_bus.c）

在 `fw-AC63_BT_SDK/apps/src/drivers/i2c/i2c_bus.c` 中优化扫描范围：
...
**I2C地址规范：**
- `0x00-0x07` (8个): 保留地址，永远不会分配给设备
- `0x78-0x7F` (8个): 10位地址模式保留
- `0x08-0x77` (112个): 有效地址范围

**效果：**
- ✅ 减少12.5%的扫描次数（128→112）
- ✅ 节省约80ms扫描时间
- ✅ 不影响正常设备检测
...
| 项目 | 优化前 | 优化后 | 改善 |
|------|--------|--------|------|
| 单次超时时间 | ~5ms (50000次) | ~1ms (10000次) | ↓ 80% |
| 扫描地址数 | 128个 | 112个 | ↓ 12.5% |
| I2C扫描总耗时 | ~640ms | ~112ms | ↓ 82.5% |
| 整体启动时间 | ~6秒 | ~0.3秒 | **↓ 95%** |

### A.4 注意事项

1. **超时值调整**
   - 如需进一步加快启动，可将超时值改为5000
   - 但可能影响慢速设备的通信稳定性
   - 建议根据实际测试结果调整

2. **跳过I2C扫描**
   - 如果已知设备地址，可以在示例代码中注释掉 `i2c_bus_scan()`
   - 可再节省~100ms启动时间
   - 适合生产环境，开发阶段建议保留用于调试

3. **日志输出**
   - 超时信息通过 `printf` 输出到串口
   - 格式：`[IIC_HW] TX timeout! byte=0xXX`
   - 便于定位I2C通信问题
...
**更新记录：**
- v1.1 (2026-05-11): 添加 I2C驱动优化说明章节
