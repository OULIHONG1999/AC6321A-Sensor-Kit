...
const struct low_power_param power_param = {
    .config = TCFG_LOWPOWER_LOWPOWER_SEL, // 0：sniff时芯片不进入低功耗
                                          // 1：sniff时芯片进入powerdown
    .btosc_hz = TCFG_CLOCK_OSC_HZ, // 外接晶振频率
    .delay_us =
        TCFG_CLOCK_SYS_HZ / 1000000L, // 提供给低功耗模块的延时(不需要需修改)
    .btosc_disable = TCFG_LOWPOWER_BTOSC_DISABLE, // 进入低功耗时BTOSC是否保持
    .vddiom_lev = TCFG_LOWPOWER_VDDIOM_LEVEL, // 强VDDIO等级,可选：2.0V  2.2V  2.4V
                                              // 2.6V  2.8V  3.0V  3.2V  3.6V
    .vddiow_lev =
        TCFG_LOWPOWER_VDDIOW_LEVEL, // 弱VDDIO等级,可选：2.1V  2.4V  2.8V  3.2V
...
/************************** CHARGE config****************************/
    ...
    /*ldo5v拔出过滤值，过滤时间 = (filter*2 +
     20)ms,ldoin<0.6V且时间大于过滤时间才认为拔出
     对于充满直接从5V掉到0V的充电仓，该值必须设置成0，对于充满由5V先掉到0V之后再升压到xV的
     充电仓，需要根据实际情况设置该值大小*/
    .ldo5v_off_filter = 100, .ldo5v_on_filter = 50, .ldo5v_keep_filter = 220,
...
const struct iokey_port iokey_list[] = {
    {
        .connect_way = TCFG_IOKEY_KEY1_CONNECT_WAY,       // IO按键的连接方式
        .key_type.one_io.port = TCFG_IOKEY_KEY1_ONE_PORT, // IO按键对应的引脚
        .key_value = TCFG_IOKEY_KEY1_ONE_PORT_VALUE,      // 按键值
    },

    {
        .connect_way = TCFG_IOKEY_KEY2_CONNECT_WAY,
        .key_type.one_io.port = TCFG_IOKEY_KEY2_ONE_PORT,
        .key_value = TCFG_IOKEY_KEY2_ONE_PORT_VALUE,
    },

    {
        .connect_way = TCFG_IOKEY_KEY3_CONNECT_WAY,
        .key_type.one_io.port = TCFG_IOKEY_KEY3_ONE_PORT,
        .key_value = TCFG_IOKEY_KEY3_ONE_PORT_VALUE,
    },
    {
        .connect_way = TCFG_IOKEY_KEY4_CONNECT_WAY,
        .key_type.one_io.port = TCFG_IOKEY_KEY4_ONE_PORT,
        .key_value = TCFG_IOKEY_KEY4_ONE_PORT_VALUE,
    },
};
...
const struct sys_time def_alarm = {
    // 初始一下目标时间，即闹钟时间
    .year = 2050, .month = 1, .day = 1, .hour = 0, .min = 0, .sec = 0,
};

extern void alarm_isr_user_cbfun(u8 index);
RTC_DEV_PLATFORM_DATA_BEGIN(rtc_data).default_sys_time = &def_sys_time,
...
/************************** PWM_LED ****************************/
        ...
        LED_PLATFORM_DATA_BEGIN(pwm_led_data)
            .io_mode =
        TCFG_PWMLED_IOMODE, // 推灯模式设置:支持单个IO推两个灯和两个IO推两个灯
...
u8 get_power_on_status(void) {
  ...
  if (iokey_data.enable) {
    if (gpio_read(power_io_list->key_type.one_io.port) ==
        power_io_list->connect_way) {
      return 1;
    }
  }
#endif
...
#include "uart_update.h"
    uart_update_cfg update_cfg = {.rx = UART_UPDATE_RX_PORT,
...
#if TCFG_IOKEY_ENABLE
  port_protect(port_group, TCFG_IOKEY_KEY1_ONE_PORT);
  port_protect(port_group, TCFG_IOKEY_KEY2_ONE_PORT);
  port_protect(port_group, TCFG_IOKEY_KEY3_ONE_PORT);
  port_protect(port_group, TCFG_IOKEY_KEY4_ONE_PORT);
...
#else
    .iomap = TCFG_IOKEY_KEY1_ONE_PORT, // 唤醒口选择
#endif
    .filter = PORT_FLT_2ms,
};
...
#define APP_IO_DEBUG_0(                                                        \
    i, x) //{JL_PORT##i->DIR &= ~BIT(x), JL_PORT##i->OUT &= ~BIT(x);}
#define APP_IO_DEBUG_1(                                                        \
    i, x) //{JL_PORT##i->DIR &= ~BIT(x), JL_PORT##i->OUT |= BIT(x);}
...
static void wl_audio_clk_on(void) { JL_WL_AUD->CON0 = 1; }

//-----------------------
// system check go sleep is ok
#if CONFIG_APP_AT_CHAR_COM || CONFIG_APP_AT_COM
static u8 board_is_active;
static u8 board_state_idle_query(void) { return !board_is_active; }
...
static u8 board_time_to_idle(void) { board_is_active = 0; }
...
static void port_wakeup_callback(u8 index, u8 gpio) {
  ...
  case AT_UART_PORT_ID:
    if (!board_is_active) {
      board_is_active = 1;
      putchar('W');
      sys_timeout_add(0, board_time_to_idle,
                      10000); // delay 给uart 指令退出低功耗
    }
    break;
#endif
  default:
...
//   log_info("Power init : %s", __FILE__);
  ...
  power_set_callback(TCFG_LOWPOWER_LOWPOWER_SEL, sleep_enter_callback,
                     sleep_exit_callback, board_set_soft_poweroff);
